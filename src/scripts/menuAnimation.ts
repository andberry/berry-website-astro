...
export const desktopMenuAnimation = () => {
  ...
  mainTl.fromTo(desktopNavLinks, {autoAlpha: 0, y: 3}, {autoAlpha: 1, y: 0, ease: 'power3.out', duration: 0.3, stagger: 0.06})
}
...
export const setupMobileMenuAnimation = () => {
  ...
  const tl = gsap.timeline({paused: true, delay: 1, defaults: animationsDefaults})
...
export const setupLogoAnimation = () => {
  ...
  const logoTl = gsap.timeline({repeat: -1, delay: 0.5, defaults: animationsDefaults});
  logoTl.fromTo(logo,{y: '100%', opacity: 0}, {opacity: 1, y: 0, ease: 'expo.out'});
  logoTl.to(logo,{opacity: 0, y: '100%', duration: 0.3, ease: 'expo.inOut'}, '>+9')
